...
upStreamStretch = int(sys.argv[5]); # in bp
maxX = int(sys.argv[6]);            # in bp, if -1 maxX <= max(feature length), see below
tickSpacing = int(sys.argv[7]);     # in bp
plotTitle = sys.argv[8];
...
for row in feaFileReader:
    feaName, feaChrNo, feaStrand, feaStart, feaEnd = row;
...
# preallocate the results array, bins and 1 extra cols for feature length
avgFeatureMatScrs = np.empty((len(feaStarts),\
                          (int(math.ceil(float(max(feaEnds - feaStarts))/float(stepWidth)) + upStreamStretch/stepWidth + 1))),\
                         );
...
for chr in range(min(feaChrNos),max(feaChrNos) + 1):
    ...
    for j in range(0,len(currStarts)):
        ...
        if feaStrand == 1:
            ...
            # write feature length in last col of the array
...
# assign names and data format to all cols of the results array and sort array by feature length (last column)
...
avgFeatureMatScrs.sort(axis = 0, order = sortByLengthCol);
...
# remove custom array dtype used for sorting and plot avgScrs (without the last col which stores the length info)
imgplot = imshow(avgFeatureMatScrs.view(np.float)[:,0:-1], aspect='auto', interpolation='nearest', origin='lower', vmin = -cmLim, vmax = cmLim);
...
savefig('chromatra_l_tmp_plot', format=outFileFormat);
data = file('chromatra_l_tmp_plot', 'rb').read();
fp = open(outFileName, 'wb');
fp.write(data);
fp.close();
os.remove('chromatra_l_tmp_plot');
